#!/usr/bin/perl -w
# conlleval_rev.perl: 评估处理SIGHAN NER任务的结果
...
my $correct;              # current corpus chunk tag (I,O,B) 目前语料库块标记
my $correctChunk = 0;     # number of correctly identified chunks 正确识别的块的数目
my $correctTags = 0;      # number of correct chunk tags   正确的块标记的数量
my $correctType;          # type of current corpus chunk tag (NP,VP,etc.) 当前主体块标记的类型
my $delimiter = " ";      # field delimiter
my $FB1 = 0.0;            # FB1 score (Van Rijsbergen 1979) FB1得分
my $firstItem;            # first feature (for sentence boundary checks)
my $foundCorrect = 0;     # number of chunks in corpus  语料库中的块数
my $foundGuessed = 0;     # number of identified chunks 语料中被识别的块数
my $guessed;              # current guessed chunk tag   当前猜测块的标记
my $guessedType;          # type of current guessed chunk tag   当前猜测块标记的类型
...
my $tokenCounter = 0;     # token counter (ignores sentence breaks) 令牌计数器(忽略句子中断)
...
if (not $latex) {
   ...
   $precision = 100*$correctChunk/$foundGuessed if ($foundGuessed > 0); # 正确识别的块的数目/语料中被识别的块数
   $recall = 100*$correctChunk/$foundCorrect if ($foundCorrect > 0);    # 正确识别的块的数目/语料库中的块数
   ...
   if ($tokenCounter>0) {
      printf "accuracy: %6.2f%%; ",100*$correctTags/$tokenCounter;  #正确识别的块的数目/令牌计数
...
sub endOfChunk {
   ...
   if ( $prevTag eq "M" and $tag eq "B" ) { $chunkEnd = $true; }
   if ( $prevTag eq "M" and $tag eq "0" ) { $chunkEnd = $true; }

   if ( $prevTag eq "E" and $tag eq "E" ) { $chunkEnd = $true; }
   if ( $prevTag eq "E" and $tag eq "M" ) { $chunkEnd = $true; }
...
sub startOfChunk {
   ...
   if ( $prevTag eq "M" and $tag eq "B" ) { $chunkStart = $true; }
   if ( $prevTag eq "0" and $tag eq "B" ) { $chunkStart = $true; }
   if ( $prevTag eq "0" and $tag eq "M" ) { $chunkStart = $true; }

   if ( $prevTag eq "E" and $tag eq "E" ) { $chunkStart = $true; }
   if ( $prevTag eq "E" and $tag eq "M" ) { $chunkStart = $true; }
